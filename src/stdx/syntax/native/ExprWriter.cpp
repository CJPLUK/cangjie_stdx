...
flatbuffers::Offset<NodeFormat::Expr> NodeWriter::SerializeLitConstExpr(AstExpr expr)
{
    ...
    auto fbSiExprs = 
        FlatVectorCreateHelper<NodeFormat::Expr, Expr, AstExpr>(std::vector<OwnedPtr<Expr>>(), &NodeWriter::SerializeExpr);
    if (litConstExpr->siExpr != nullptr) {
        fbSiExprs = FlatVectorCreateHelper<NodeFormat::Expr, Expr, AstExpr>(litConstExpr->siExpr.get()->strPartExprs, &NodeWriter::SerializeExpr);
    }
...
flatbuffers::Offset<NodeFormat::Expr> NodeWriter::SerializeInterpolationExpr(AstExpr expr)
{
    auto interpolationExpr = RawStaticCast<const InterpolationExpr*>(expr);
    if (interpolationExpr == nullptr) {
        auto fbInterpolationExpr = flatbuffers::Offset<NodeFormat::InterpolationExpr>();
        return NodeFormat::CreateExpr(
            builder, emptyNodeBase, NodeFormat::AnyExpr_INTERPOLATION_EXPR, fbInterpolationExpr.Union());
    }
    ...
    auto fbInterpolationExpr = NodeFormat::CreateInterpolationExpr(builder, fbNodeBase, &dollarPos, block);
    return NodeFormat::CreateExpr(builder, fbNodeBase, NodeFormat::AnyExpr_INTERPOLATION_EXPR, fbInterpolationExpr.Union());
}
...
flatbuffers::Offset<NodeFormat::FuncArg> NodeWriter::SerializeFuncArg(AstFuncArg funcArg)
{
    ...
    return NodeFormat::CreateFuncArg(
...
flatbuffers::Offset<NodeFormat::Expr> NodeWriter::SerializeResumeExpr(AstExpr expr)
{
    ...
    auto fbExpr = SerializeExpr(resumeExpr->expr);
...
flatbuffers::Offset<NodeFormat::NodeBase> NodeWriter::SerializeNodeBase(AstNode node, SourceManager* sm)
{
    if (node == nullptr) {
